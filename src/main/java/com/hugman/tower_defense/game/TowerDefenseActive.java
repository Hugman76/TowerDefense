...
public class TowerDefenseActive {
    private final TowerDefenseConfig config;

    public final GameSpace gameSpace;
    private final TowerDefenseMap gameMap;

    // TODO replace with ServerPlayerEntity if players are removed upon leaving
    private final Object2ObjectMap<PlayerRef, TowerDefensePlayer> participants;
    private final TowerDefenseSpawnLogic spawnLogic;
    private final TowerDefenseStageManager stageManager;
    private final boolean ignoreWinState;
    private final TowerDefenseTimerBar timerBar;

    private TowerDefenseActive(GameSpace gameSpace, TowerDefenseMap map, GlobalWidgets widgets, TowerDefenseConfig config, Set<PlayerRef> participants) {
        ...
        for (PlayerRef player : participants) {
            this.participants.put(player, new TowerDefensePlayer());
        }

        this.stageManager = new TowerDefenseStageManager();
        this.ignoreWinState = this.participants.size() <= 1;
        this.timerBar = new TowerDefenseTimerBar(widgets);
    }

    public static void open(GameSpace gameSpace, TowerDefenseMap map, TowerDefenseConfig config) {
        gameSpace.openGame(game -> {
            Set<PlayerRef> participants = gameSpace.getPlayers().stream()
                    .map(PlayerRef::of)
                    .collect(Collectors.toSet());
            GlobalWidgets widgets = new GlobalWidgets(game);
            TowerDefenseActive active = new TowerDefenseActive(gameSpace, map, widgets, config, participants);
    ...
    private void tick() {
        ...
        TowerDefenseStageManager.IdleTickResult result = this.stageManager.tick(time, gameSpace);
